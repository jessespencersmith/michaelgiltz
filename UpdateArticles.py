...
def main(): 
  print
  print
  print 'michaelgiltz.com synchronization script'
  print
  print 'Checking for incorrectly formatted names:'
  os.chdir("/Users/michaelgiltzold/documents/giltzweb")

  #Username and Password for Michael's FTP account
  host = "ftp.michaelgiltz.com"
  username = "webmanager@michaelgiltz.com"
  password = "ebullient-frozen-paw-vine12"
  ...
  print 'Filtered remote directory'
  #Call routine to sync current directory with web directory
  #Note: Must be changed if host changes, duh.
  print 'Syncing article lists:'  
  SyncArticles(host, username, password)
  print 'Copying all server HTML files to local:'
  print '(Note that edits to HTML need to be saved to server before running this script. Too late now!)'
  ...
  print 'Putting HTML files up on server:'
